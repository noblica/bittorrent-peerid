...
exports.peerid = peerId => {
    ...
    if (utils.isAzStyle(peerId)) {
        if ((client = getAzStyleClientName(peerId))) {
            ...
            if (client.startsWith("ZipTorrent") &&
...
const VER_AZ_THREE_DIGITS = (v) => `${v[0]}.${v[1]}.${v[2]}`;
...
const VER_AZ_FOUR_DIGITS = (v) => `${v[0]}.${v[1]}.${v[2]}.${v[3]}`;
const VER_AZ_TWO_MAJ_TWO_MIN = (v) => `${v[0] + v[1]}.${v[2]}${v[3]}`;
const VER_AZ_SKIP_FIRST_ONE_MAJ_TWO_MIN = (v) => `${v[1]}.${v[2]}${v[3]}`;
