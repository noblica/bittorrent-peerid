export const isAzStyle = (peerId: string) => {
	if (peerId[0] !== "-") {
		return false;
	}
	if (peerId[7] === "-") {
 ...
 */
export const isShadowStyle = (peerId: string) => {
	if (
		peerId[5] !== "-" ||
		!isLetter(peerId[0]) ||
...
export const isMainlineStyle = (peerId: string) => {
	/**
...
export const isPossibleSpoofClient = (peerId: string) => {
	return peerId.endsWith("UDP0") || peerId.endsWith("HTTPBT");
};

export const decodeNumericValueOfByte = (b: number, minDigits = 0) => {
	let result = `${b & 0xff}`;
...
export const getAzStyleVersionNumber = (
	peerId: string,
...
export const decodeBitSpiritClient = (peerId: string, buffer: Buffer) => {
...
export const decodeBitCometClient = (peerId: string, buffer: Buffer) => {
	let modName = "";
	...
	return {
		...
		version: `${majVersion}.${decodeNumericValueOfByte(
			buffer[5],
			minVersionLength,
		)}`,
	};
...
export const identifyAwkwardClient = (buffer: Buffer) => {
