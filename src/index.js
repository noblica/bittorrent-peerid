...
module.exports = peerId => {
	...
	if (Buffer.isBuffer(peerId)) {
		...
		throw new Error(
			`Invalid peerId must be Buffer or hex string: ${peerId}`,
		);
	}

	if (buffer.length !== 20) {
		throw new Error(
			`Invalid peerId length (hex buffer must be 20 bytes): ${peerId}`,
		);
	}
	...
	if (utils.isPossibleSpoofClient(peerId)) {
		if ((client = utils.decodeBitSpiritClient(peerId, buffer)))
			return client;
		if ((client = utils.decodeBitCometClient(peerId, buffer)))
			return client;
	...
	if ((client = utils.identifyAwkwardClient(buffer))) {
		return client;
	}
...
const VER_AZ_FOUR_DIGITS = (
	v, // "1.2.3.4"
) => `${v[0]}.${v[1]}.${v[2]}.${v[3]}`;
const VER_AZ_TWO_MAJ_TWO_MIN = (
	v, // "12.34"
) => `${v[0] + v[1]}.${v[2]}${v[3]}`;
const VER_AZ_SKIP_FIRST_ONE_MAJ_TWO_MIN = (
	v, // "2.34"
) => `${v[1]}.${v[2]}${v[3]}`;
